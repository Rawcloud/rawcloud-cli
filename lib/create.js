...
function isValidRepoUrl(repoUrl) {
	...
	try {
		...
		return ['http:', 'https:', 'ssh:', 'git:'].includes(parsedUrl.protocol)
	} catch (error) {
...
async function resolveTargetPath(name, options) {
	if (path.isAbsolute(name) || name.split(/[\\/]/).includes('..')) {
		throw new Error('项目名称不能包含路径分隔符或 ".."')
	}
...
async function cloneTemplate(repoUrl, targetPath, targetName) {
	const finishProgress = startDownloadProgress()

	try {
		const git = simpleGit(process.cwd())
		await git.clone(repoUrl, targetName)
		finishProgress()
...
module.exports = async function(name, options = {}) {
	try {
		...
		let repo
		if (options.repo) {
			if (!isValidRepoUrl(options.repo)) {
				throw new Error('仓库地址格式无效，请使用 HTTPS、SSH、Git 协议或 git@host:path 格式')
			}
			repo = options.repo
		} else {
			const selectedRepo = await getRepos()
			repo = `https://gitee.com/rawcloud/${selectedRepo}.git`
		}
